...
├── .gitignore
│
└── VBA_v6.5/              ← v6.5 版 VBA 原始碼（目前使用）
    ├── modConfig.bas      ← 設定檔化 InitPortfolios
    ├── modProcess.bas     ← 核心資料處理引擎（SyncDataByPrimaryKey / FillTradeTicket / Phase 5）
    ├── modMain.bas        ← 主流程入口（RunDaily / ExportTradeTicket / NukeGhostData）
    └── modCompliance.bas  ← 合規檢核引擎（6 項規則 + 動態 header 驗證）
```
